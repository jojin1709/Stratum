...
// and JS integers are exact below 2^53, which covers every realistic count or id here.
types.setTypeParser(20, (v) => {
  const n = Number(v);
  return Number.isSafeInteger(n) ? n : v;
});
// numeric/decimal stay strings on purpose — silently lossy floats are worse than strings.

/** Postgres error codes that are the caller's fault, not ours. */
...
export interface PostgresAdapterOptions {
  ...
  /**
   * Schemas unqualified SQL resolves against. Defaults to `public` only.
   *
   * Postgres puts `"$user"` first in the default search_path, so a database user
   * named after an existing schema silently captures every unqualified CREATE TABLE.
   * Stratum reserves a schema called `stratum`, and pinning the path removes that
   * whole class of bug.
   */
  searchPath?: string[];
}
...
export class PostgresAdapter implements DatabaseAdapter {
  ...
  constructor(opts: PostgresAdapterOptions) {
    ...
    // An unhandled pool error would otherwise take the whole process down.
    this.pool.on('error', () => {});
  }
  ...
  async query<T = Record<string, unknown>>(sql: string, params: unknown[] = []): Promise<QueryResult<T>> {
    ...
    try {
      ...
      // A multi-statement script (migrations, bootstrap) comes back as an array of
      // results. The last one is the meaningful answer; the rest are DDL acks.
